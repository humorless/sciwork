...
**在 Macbook M1 筆電上的實際耗時**（包含查詢編譯 + 演算法執行）
...
**總耗時**：~30 秒（6 個算法）| **記憶體**：~100 MB

---

# Python + NetworkX 實測耗時（M1 筆電）

**同樣的 340 萬邊，用 Python + NetworkX（實測）**：
...
| **4 個算法執行** | **213.78 秒** |
| ├─ PageRank | 74.4 秒 |
| ├─ Degree Centrality | 6.86 秒 |
| ├─ WCC | 38.32 秒 |
| └─ SCC | 94.2 秒 |
| **總耗時** | **265.54 秒** |

**記憶體峰值**：3.39 GB（整個圖載入內存）

---

# 實測對比：Ladybug vs. Python + NetworkX

**M1 筆電，340 萬邊，同 4 個算法**

| 階段 | Ladybug | Python + NetworkX | 倍數 |
|------|---------|------------------|------|
| **I/O** | — | 1.69 秒 | — |
| **建圖/準備** | — | **50.07 秒** ⚠️ | — |
| **4 個算法** | **~29 秒** | **213.78 秒** | **7.4x 慢** |
| **總耗時** | **~29 秒** | **265.54 秒** | **9.1x 慢** |
| **記憶體** | ~100 MB | 3.39 GB | **34x 多** |

**瓶頸分析**：
- 建圖成本高（50 秒），光是為了準備資料結構就要花這麼長時間
- 資料必須全部載入內存（3.39 GB 峰值）
- 單線程執行無法並行化
